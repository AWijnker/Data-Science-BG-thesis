...
without_category_plot <- top_10_without_category %>% ggplot(aes(x = reorder(apps_without_category, Freq), y = Freq)) + 
...
app_categories <- app_categories[!(app_categories$application == "com.whatsapp" | app_categories$application == "com.android.systemui"| 
                                     app_categories$application == "com.instagram.android"| app_categories$application == "com.snapchat.android"| 
                                     app_categories$application == "com.android.chrome"| app_categories$application == "com.facebook.katana"| 
                                     app_categories$application == "com.spotify.music"| app_categories$application == "com.google.android.youtube" 
                                     | app_categories$application == "com.ethica.logger" #| app_categories$application == "com.facebook.orca"
                                     ),]
...
app_categories <- app_categories %>% add_row(application = "com.android.systemui", count = 100, name = "System UI", 
  ...
  add_row(application = "com.ikeyboard.theme.blue.love.heart", count = 100, 
          ...
          category = "Phone_Personalization", better_category_hybrid = "Phone_Personalization", better_category = "Phone_Personalization") %>%
...
phone_use_cat_clean <- phone_use_cat_clean[(phone_use_cat_clean$stressed == "0" | phone_use_cat_clean$stressed == "1" | 
...
phone_use_cat_clean$better_category[phone_use_cat_clean$better_category == "Job_Search"] <- "Social_Networking"
phone_use_cat_clean$better_category[which(phone_use_cat_clean$better_category == "Messages")] <- "Messaging"
phone_use_cat_clean$better_category[which(phone_use_cat_clean$better_category == "Music_&_Audio")] <- "Music_Audio"
...
phone_use_cat_clean$duration_cat[phone_use_cat_clean$duration > 5 & phone_use_cat_clean$duration <= 57.5] <- "short"
phone_use_cat_clean$duration_cat[phone_use_cat_clean$duration > 57.5 & phone_use_cat_clean$duration <= 300] <- "middle"
phone_use_cat_clean$duration_cat[phone_use_cat_clean$duration > 300 & phone_use_cat_clean$duration <= 1200] <- "long"
...
phone_use_morning_stress$stressed_morning <- as.factor(round(phone_use_morning_stress$stressed_morning , 0))
...
phone_use_evening_stress$stressed_evening <- as.factor(round(phone_use_evening_stress$stressed_evening , 0))
...
boa_train_x <- select(boa_train, -(stressed))
boa_train_y <- select(boa_train, stressed)
write.csv(boa_train_x, "method/boa_cat_train_x.csv", sep=",", row.names = FALSE)
write.csv(boa_train_y, "method/2boa_cat_train_y.csv", sep=",", row.names = FALSE)


boa_test_x <- select(boa_test, -(stressed))
boa_test_y <- select(boa_test, stressed)
write.csv(boa_test_x, "method/boa_cat_test_x.csv", sep=",", row.names = FALSE)
write.csv(boa_test_y, "method/boa_cat_test_y.csv", sep=",", row.names = FALSE)
...
cspade_input$SIZE <- sapply(cspade_input$apps, function(x) length(unlist(strsplit(as.character(x), "\\W+"))))
...
# run cspade
cspade_stress_categorical <- cspade(spade_stress_ready, parameter = list(support = 0.9, maxsize = 1, maxlen = 6), control = list(verbose = TRUE))
...
spm_daily_patterns <- read.delim("input/spm_daily_patterns_0.8_4.csv", stringsAsFactors = FALSE, sep = ",")
...
#spm_daily_patterns <- add_case(spm_daily_patterns, sequence = "stressed", support = "0", .before = TRUE)
spm_daily_patterns <- add_case(spm_daily_patterns, sequence = "apps", support = "0", .before = TRUE)
spm_daily_patterns <- add_case(spm_daily_patterns, sequence = "size", support = "0", .before = TRUE)
spm_daily_patterns <- add_case(spm_daily_patterns, sequence = "response_date", support = "0", .before = TRUE)
spm_daily_patterns <- add_case(spm_daily_patterns, sequence = "user_id", support = "0", .before = TRUE)
...
write.csv(spm_daily_combined, "method/spm_daily_combined_0.8_4.csv", row.names = FALSE, quote = FALSE)
